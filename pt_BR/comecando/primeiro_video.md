...
There are a few methods we need to know about Scene before studying Objects, which is the [next topic](first_video.md/#adding-shapes). There are three methods in the `Scene` class:

### The `add(*objects: Object)` method
...
So, when running `python ./my_file.py`, we get this:
